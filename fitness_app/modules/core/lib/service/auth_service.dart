...
class AuthService {
  ...
  BaseResponseData<dynamic> fetchCurrentUserDoc(dynamic model) async {
    try {
      final userInfo =
          await _firestore.collection(userCollection).doc(_auth.currentUser?.uid).get();
      if (userInfo.data() != null) {
        final user = model.fromJson(userInfo.data()!);
        return Right(user);
      } else {
  ...
  Future<void> setUser({required dynamic model}) async {
    return _firestore
  ...
  BaseVoidData updateData<T>({required BaseModel<T> model}) async {
    try {
      await _firestore
          .collection(userCollection)
          .doc(_auth.currentUser!.uid)
  ...
  BaseVoidData createPerson({required dynamic model}) async {
    try {
  ...
  BaseVoidData signInWithEmailAndPassword({required dynamic model}) async {
    try {
      final credential =
